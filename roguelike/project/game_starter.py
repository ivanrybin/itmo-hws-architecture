...
from engine.serialization import load_game
...
def starter(default_screen=True, fov_mode=False, debug=False):
    ...
    if os.path.isfile('media/GAME_SAVE.json'):
        ...
        if new_or_save == '2':
            return load_game('media/GAME_SAVE.json', is_save=True, scr_wd=scr_wd, scr_ht=scr_ht, fov_mode=fov_mode, debug=debug)
        elif new_or_save == '1':
            return load_game('media/SD_MAP.json', is_save=False, scr_wd=scr_wd, scr_ht=scr_ht, fov_mode=fov_mode, debug=debug)
    ...
    if new_or_load == '1':
        return load_game('media/SD_MAP.json',
                         is_save=False,
                         scr_wd=scr_wd,
                         scr_ht=scr_ht,
                         fov_mode=fov_mode,
